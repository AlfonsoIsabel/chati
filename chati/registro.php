<?php
	...
	if ($_POST) {
		...
		*/
		...
		if ((isset($correo) && !empty($correo)) && (isset($nombre) && !empty($nombre)) && (isset($clave) && !empty($clave)) && (isset($reclave) && !empty($reclave))) {
			if ($clave == $reclave) {
				...
				if (mysqli_num_rows($queryExisteCorreo) > 0) {
					...
					$sqlNuevoUsuario = "
						INSERT INTO usuario
		                VALUES (null, '".$nombre."', '".$correo."', '".password_hash($clave, PASSWORD_DEFAULT)."',
		                    '".$sexo."', 'https://i.pinimg.com/originals/2f/a8/9a/2fa89a127b2ac70c76120da902d3c4a7.jpg',1,1,1,2);
					";
					...
					if (!$queryNuevoUsuario) {
					    ...
					    while ($rowRecienRegistrado = mysqli_fetch_assoc($queryRecienRegistrado)){
					        ...
					        $mensaje = "Bienvenid@ al chat de KOALATOMATO.COM.<br> Para validar tu cuenta pulsa <a href='http://www.motoraidermad.com?idUsuario=".$rowRecienRegistrado['id_usuario']."' target='blank'>aquí</a>";
